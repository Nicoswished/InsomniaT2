...
public class ItemBuilder {
    ...
    public ItemBuilder(ItemStack itemStack) {
        ...
        public ItemBuilder addLore(String... lore) {
            if (this.is == null) {
                throw new IllegalStateException("El ItemStack no puede ser nulo.");
            }
            if (lore == null || lore.length == 0) {
                throw new IllegalArgumentException("El lore no puede ser nulo o vacío.");
            }

            // Obtener el ItemMeta
            this.im = this.is.getItemMeta();
            if (this.im == null) {
                throw new IllegalStateException("El ItemMeta no puede ser nulo.");
            }

            // Obtener el lore actual o crear una nueva lista
            List<String> finalLore = this.im.hasLore() ? this.im.getLore() : new ArrayList<>();
            if (finalLore == null) {
                finalLore = new ArrayList<>();
            }

            // Agregar las líneas nuevas
            for (String s : lore) {
                if (s != null && !s.isEmpty()) {
                    finalLore.add(chatcolor(s));
                }
            }

            // Aplicar el lore al ItemMeta y luego al ItemStack
